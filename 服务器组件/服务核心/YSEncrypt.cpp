...
//生成密文
bool CYSEncrypt::MD5Encrypt(LPCTSTR pszSourceData, TCHAR szMD5Result[LEN_MD5])
{
	//变量定义
	CMD5Aide MD5Aide;
	UCHAR szResult[16];
	...
	for (INT i = 0; i < 16; i++)
	{
		_stprintf_s(&szMD5Result[i * 2], (LEN_MD5-i*2), TEXT("%02X"), szResult[i]);
	}
...
//生成密文
bool CYSEncrypt::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	//变量定义
	...
	for (WORD i=0;i<wTimes;i++)
	{
		...
		swprintf(szEncrypData + i * 8, MAX_ENCRYPT_LEN - i*8, L"%04X%04X", wRandKey[i%ENCRYPT_KEY_LEN], wTempCode);
	}
...
//解开密文
bool CYSEncrypt::XorCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//设置结果
	pszSourceData[0]=0;
...
//生成密文
bool CYSEncrypt::MapEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	//效验参数
...
//解开密文
bool CYSEncrypt::MapCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//效验参数
	ASSERT(wMaxCount>lstrlen(pszEncrypData));
	ASSERT((pszEncrypData!=NULL)&&(pszSourceData!=NULL));
...
//加密数据
WORD CYSEncrypt::EncryptBuffer(BYTE pcbDataBuffer[], WORD wDataSize, WORD wBufferSize, bool bClient/* = true*/)
{
	//效验参数
	...
	//调整长度
	WORD wEncryptSize = wDataSize - (bClient ? sizeof(TCP_Command) : sizeof(TCP_Info));
	WORD wSnapCount = 0;
	...
	{
		BYTE cbData = pcbDataBuffer[i];
		cbCheckCode += cbData;
		pcbDataBuffer[i] = g_SendByteMap[cbData];
	}
...
//解密数据
WORD CYSEncrypt::CrevasseBuffer(BYTE pcbDataBuffer[], WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize >= sizeof(TCP_Head));
	...
	{
		BYTE cbData = pcbDataBuffer[i];
		pcbDataBuffer[i] = g_RecvByteMap[cbData];
