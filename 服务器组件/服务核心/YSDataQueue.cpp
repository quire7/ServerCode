...
//负荷信息
VOID CYSDataQueue::GetBurthenInfo(tagBurthenInfo & BurthenInfo)
{
	//设置变量
...
//插入数据
bool CYSDataQueue::InsertData(WORD wIdentifier, VOID * pBuffer, WORD wDataSize)
{
	//变量定义
...
//插入数据
bool CYSDataQueue::InsertData(WORD wIdentifier, tagDataBuffer DataBuffer[], WORD wDataCount)
{
	//变量定义
	tagDataHead DataHead;
	ZeroMemory(&DataHead,sizeof(DataHead));
...
//获取数据
bool CYSDataQueue::DistillData(tagDataHead & DataHead, VOID * pBuffer, WORD wBufferSize)
{
	//效验变量
...
//删除数据
VOID CYSDataQueue::RemoveData(bool bFreeMemroy)
{
	//设置变量
	m_dwDataSize=0L;
	m_dwInsertPos=0L;
...
//调整存储
bool CYSDataQueue::RectifyBuffer(DWORD dwNeedSize)
{
	try
	{
		//缓冲判断
