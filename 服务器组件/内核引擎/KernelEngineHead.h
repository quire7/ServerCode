...
public:
	...
	//关闭记录
	virtual VOID CleanupResults() = NULL;
	//连接信息
	virtual bool SetDBInfo(DWORD dwDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword)=NULL;
	//连接信息
	virtual bool SetDBInfo(LPCTSTR szDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword)=NULL;

	//状态接口
public:
	//是否打开
	virtual BOOL IsOpened() = NULL;
	//检测重连
	virtual BOOL CheckAndReconnect(DWORD dwCheckInterval = 1000) = NULL;

	//事务接口
public:
	//开始事务
	virtual BOOL BeginTransaction() = NULL;
	//停止事务
	virtual BOOL CommitTransaction() = NULL;
	//回滚数据
	virtual BOOL RollbackTransaction() = NULL;

public:
	//设置自动提交模式
	virtual BOOL SetAutoCommit(BOOL bAutoCommit) = NULL;
	//选择数据库
	virtual BOOL SelectDatabase(const std::string& strDatabaseName) = NULL;

	//参数接口
public:
	//插入参数
	virtual void AddParameter(bool bValue) = NULL;
	//插入参数
	virtual void AddParameter(BYTE cbValue) = NULL;
	//插入参数
	virtual void AddParameter(WORD wValue) = NULL;
	//插入参数
	virtual void AddParameter(int iValue) = NULL;
	//插入参数
	virtual void AddParameter(DWORD dwValue) =NULL;
	//插入参数
	virtual void AddParameter(LONG lValue) = NULL;
	//插入参数
	virtual void AddParameter(LONGLONG llValue) = NULL;
	//插入参数
	virtual void AddParameter(SYSTEMTIME& st) = NULL;
	//插入参数
	virtual void AddParameter(LPCTSTR lpsValue) = NULL;
	//插入参数
	virtual void AddBinaryParameter(LPCTSTR lpsValue, int nStrSize) = NULL;
	//插入输出参数
	virtual void AddParameterOutput(const char* strOutput) = NULL;
	//清除参数
...
public:
	//存储过程
	virtual BOOL ExecuteProcess(const std::string &strSQL) = NULL;
	//执行语句
	virtual BOOL ExecuteSQL(const std::string& strSQL) = NULL;
	//获取最后插入数据ID
	virtual __int64 GetLastInsertID() = NULL;

	virtual MYSQL* GetMYSQL() = NULL;
};
