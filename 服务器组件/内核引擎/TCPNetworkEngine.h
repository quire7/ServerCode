...
enum eWSStatus
{
	eWSStatus_Invalid,		//无效;
	eWSStatus_Connect,		//链接状态;
};
...
struct CWebSocketFrameHeader
{
	...
	BYTE				cbMask;					//掩码标记;
	BYTE				cbopcode;				//操作码;
	BYTE				cbPayloadlen;			//传输数据的长度位(以字节的形式表示：7位、7+16位、或者7+64位。);
	SCORE				payloadLen;				//64长度位的传输数据;
	BYTE				cbMaskingKey[4];		//掩码;
};

class CWebSocket
{
public:
	BYTE				m_cbWSStatus;						//WS状态;
	char				m_szHandShakeResponse[1024];		//握手返回协议;
