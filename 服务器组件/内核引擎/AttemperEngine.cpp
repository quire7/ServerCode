...
void CAttemperEngine::OnQueueServiceSink(WORD wIdentifier, void * pBuffer, WORD wDataSize)
{
	...
	case EVENT_TIMER:			//定时器事件
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception,TEXT("定时器处理异常, TimerID:%u, Param:%ld"), pTimerEvent->dwTimerID, pTimerEvent->dwBindParameter);
			}
	...
	case EVENT_DATABASE:		//数据库事件
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception,TEXT("数据库处理异常, RequestID:%u"), pDataBaseEvent->wRequestID);
			}
			return;
		}
	...
	case EVENT_TCP_NETWORK_ACCEPT:	//网络应答事件
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("OnEventTCPNetworkBind异常:dwClientIP=%u, dwSocketID=%u"), pSocketAcceptEvent->dwClientAddr, pSocketAcceptEvent->dwSocketID);
			}
			if ( !bSuccess )
			{
	...
	case EVENT_TCP_NETWORK_READ:	//网络读取事件
		{
			...
			try
			{
				...
				if ( !bSuccess )
				{
					CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("消息处理失败, MainID:%u, SubID:%u"), pSocketReadEvent->Command.wMainCmdID, pSocketReadEvent->Command.wSubCmdID);
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("消息处理异常, MainID:%u, SubID:%u"), pSocketReadEvent->Command.wMainCmdID, pSocketReadEvent->Command.wSubCmdID);
				m_pTCPNetworkEngine->CloseSocket(pSocketReadEvent->dwSocketID);
			}
			return;
		}
		...
		case EVENT_TCP_NETWORK_SHUT:	//关闭读取事件
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("OnEventTCPNetworkShut异常:dwClientIP=%u, dwActiveTime=%u,dwSocketID=%u"), pNetworkShutEvent->dwClientAddr, pNetworkShutEvent->dwActiveTime, pNetworkShutEvent->dwSocketID);
			}
			if ( !bSuccess )
			{
	...
	case EVENT_TCP_SOCKET_READ:		//网络读取事件
		{
			...
			try
			{
				...
				{
					CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("消息处理失败, MainID:%d, SubID:%d"), pSocketReadEvent->Command.wMainCmdID, pSocketReadEvent->Command.wSubCmdID);
					m_pITCPSocketService->CloseSocket();
				}
			}
			catch (...)
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("消息处理异常, MainID:%d, SubID:%d"), pSocketReadEvent->Command.wMainCmdID, pSocketReadEvent->Command.wSubCmdID);
				m_pITCPSocketService->CloseSocket();
			}
			return;
		}
		...
		case EVENT_TCP_SOCKET_SHUT:	//网络关闭事件
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, TEXT("OnEventTCPSocketShut异常:wServiceID=%u, cbShutReason=%u"), pSocketShutEvent->wServiceID, pSocketShutEvent->cbShutReason);
			}
			if (!bSuccess)
			{
				m_pITCPSocketService->CloseSocket();
		...
		case EVENT_TCP_SOCKET_LINK:
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, _T("OnEventTCPSocketLink异常!wServiceID=%u,nErrorCode=%u"), pSocketLinkEvent->wServiceID, pSocketLinkEvent->nErrorCode);
			}
			if (!bSuccess)
			{
				m_pITCPSocketService->CloseSocket();
	...
	case EVENT_CONTROL:
		{
			...
			{
				ASSERT(0);
				CTraceService::TraceStringEx(TraceLevel_Exception, _T("OnEventControl异常!wIdentifier=%u,wDataSize=%u"), wIdentifier, wDataSize);
			}
	...
	{
		ASSERT(0);
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("OnEventAttemperData异常!wIdentifier=%u,wDataSize=%u"), wIdentifier, wDataSize);
	}
...
bool CAttemperEngine::OnEventDataBaseResult(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		ASSERT(0);
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("OnEventDataBaseResult失败!wRequestID=%u,dwContextID=%u,wDataSize=%u"), wRequestID, dwContextID, wDataSize);
	}
	catch (...)
	{
		ASSERT(0);
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("OnEventDataBaseResult异常!wRequestID=%u,dwContextID=%u,wDataSize=%u"), wRequestID, dwContextID, wDataSize);
	}
	return false;
}
...
bool CAttemperEngine::OnEventTCPNetworkBind(DWORD dwSocketID, DWORD dwClientAddr)
{
	...
	try
	{
		...
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟绑定处理失败!dwSocketID=%u,dwClientIP=%u"), dwSocketID, dwClientAddr);
	}
	catch (...) 
	{
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟绑定处理异常!dwSocketID=%u,dwClientIP=%u"), dwSocketID, dwClientAddr);
	}
	ASSERT(0);
	return false;
}
...
bool CAttemperEngine::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, DWORD dwActiveTime)
{
	...
	try
	{
		...
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟关闭处理失败!dwClientIP=%u,dwActiveTime=%u,dwSocketID=%u"), dwClientAddr, dwActiveTime, dwSocketID);
	}
	catch (...)	
	{
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟关闭处理异常!dwClientIP=%u,dwActiveTime=%u,dwSocketID=%u"), dwClientAddr, dwActiveTime, dwSocketID);
	}
...
bool CAttemperEngine::OnEventTCPNetworkRead(DWORD dwSocketID, TCP_Command Command, VOID * pData, WORD wDataSize)
{
	...
	try
	{
		...
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟读取处理失败!MainID=%u,SubID=%u,DataSize=%u"), Command.wMainCmdID, Command.wSubCmdID, wDataSize);
	}
	catch (...)
	{
		CTraceService::TraceStringEx(TraceLevel_Exception, _T("机器模拟读取处理异常!MainID=%u,SubID=%u,DataSize=%u"), Command.wMainCmdID, Command.wSubCmdID, wDataSize);
	}
	ASSERT(0);
	return false;
}
