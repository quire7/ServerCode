...
#define IDI_FLASH_LOVELINESS_RANKING	2							//刷新排行榜

#define TIME_LOAD_PLATFORM_PARAMETER	600000						//时间间隔
#define TIME_FLASH_LOVELINESS_RANKING	10000						//刷新排行榜时间间隔
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	m_pITimerEngine->SetTimer(IDI_FLASH_LOVELINESS_RANKING, TIME_FLASH_LOVELINESS_RANKING, TIMES_INFINITY, 0);
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	else if (IDI_FLASH_LOVELINESS_RANKING==dwTimerID)
	{
		CTime curTime = CTime::GetCurrentTime();
		//if (curTime.GetHour()>23 && curTime.GetMinute()>58 )
		{
			if (m_strLastDate != curTime.Format(_T("%Y-%m-%d")))
			{
				m_strLastDate = curTime.Format(_T("%Y-%m-%d"));
				m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_LoveLinessRanking, 0, NULL, 0);
				m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_TreasureRanking, 0, NULL, 0);
			}
		}
	...
	case SUB_CS_C_REGISTER_SERVER:	//注册房间
		{
			...
			CString strMsg;
			strMsg.Format(TEXT("ID: %d, Name: %s, IP: %s 注册成功"), pRegisterServer->wServerID, pRegisterServer->szServerName, pRegisterServer->szServerAddr);
			//提示消息
	...
	case SUB_CS_C_REGISTER_MATCH:	//注册比赛
		{
			...
			CString strMsg;
			strMsg.Format(TEXT("比赛ID: %d, Name: %s 注册成功"), pRegisterMatch->dwMatchID, pRegisterMatch->szMatchName);
			//提示消息
			CTraceService::TraceString(strMsg, TraceLevel_Normal);
	...
	case SUB_CS_C_REGISTER_WEBGATE:
		{
			...
			//消息定义
			CMD_CS_C_RegisterWebGate * pRegisterWebGate = (CMD_CS_C_RegisterWebGate *)pData;
			...
			//设置变量
			BYTE* pIP = (BYTE*)&pBindParameter->dwClientAddr;

			CString strMsg;
			strMsg.Format(TEXT("网站网关服务器(%d.%d.%d.%d)注册成功"), pIP[0], pIP[1], pIP[2], pIP[3]);
			//提示消息
			CTraceService::TraceString(strMsg, TraceLevel_Normal);
	...
	case SUB_CS_C_SEARCH_CORRESPOND:	//协调查找
		{
			...
			{
				_tcslwr_s(pSearchCorrespond->szNickName);
	...
	case SUB_CS_C_BUY_PROP_FROM_WEB:
		{
			...
			//发送通知
			SendDataToGame(INVALID_WORD, MDM_CS_WEB_SERVICE, SUB_CS_C_BUY_PROP_FROM_WEB, pBuyProp, sizeof(CMD_CS_S_BuyPropFromWeb));
