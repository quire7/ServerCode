...
BEGIN_MESSAGE_MAP(CGameServerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SERVICE_CONTROL, OnBnClickedServerControl)
...
bool CGameServerDlg::StartServerService(WORD wServerID)
{
	...
	//设置缓存参数;
	InitCacheServerParameter(DlgServerItem.m_ModuleInitParameter.GameServiceOption);
...
//获取缓存配置;
VOID CGameServerDlg::InitCacheServerParameter(tagGameServiceOption & GameServiceOption)
{
	//获取路径
	TCHAR szWorkDir[MAX_PATH] = TEXT("");
	CYSService::GetWorkDirectory(szWorkDir, CountArray(szWorkDir));
	...
	//缓存服务器
	GameServiceOption.wCacheServerPort = (WORD)IniData.ReadInt(TEXT("CacheServer"), TEXT("Port"), 6379);
	IniData.ReadString(TEXT("CacheServer"), TEXT("Addr"), NULL, GameServiceOption.szCacheServerAddr, CountArray(GameServiceOption.szCacheServerAddr));
	IniData.ReadString(TEXT("CacheServer"), TEXT("Pass"), NULL, GameServiceOption.szCacheServerPass, CountArray(GameServiceOption.szCacheServerPass));
}
...
{
	CP_ServiceControlRequest request;
	request.cbServiceControlRequestType = eServiceControlRequest_RunParameter;
	m_ServiceUnits.PostControlRequest( UI_SERVICE_CONTROL_REQUEST, &request, sizeof(request) );
}

VOID CGameServerDlg::OnBnClickedServerControl()
{
	//输出信息;
	CTraceService::TraceString(TEXT("ServerControl"), TraceLevel_Normal);
}
