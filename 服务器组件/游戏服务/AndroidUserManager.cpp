...
bool CAndroidUserManager::InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[],WORD wAndroidCount,DWORD dwBatchID)
{
	...
	for(WORD wIndex=0;wIndex<wAndroidCount;wIndex++)
	{
		...
		{
			if(m_AndroidItemConfig[wIndex].AndroidAccountsInfo.dwUserID==AndroidAccountsInfo[wIndex].dwUserID)
			{
				bAndroidExist=true;
				break;
			}
...
bool CAndroidUserManager::OnEventTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	if(dwTimerID>=IDI_ANDROID_IN && dwTimerID<IDI_ANDROID_IN+MAX_BATCH)			
	{
		...
		else
		{
			...
			if(dwRemaindTime<=ANDROID_UNIN_TIME)
			{
				...
				CTraceService::TraceStringEx(TraceLevel_Info, TEXT("本批次机器人开始退出, 批次ID=%d"), pAndroidParameter->dwBatchID);
			}
			else
			{
				//设置定时器
	...
	if(dwTimerID>=IDI_ANDROID_OUT && dwTimerID<IDI_ANDROID_OUT+MAX_BATCH)
	{
		...
		while (Position!=NULL)
		{
			...
			if (pAndroidUserItem->m_pIServerUserItem!=NULL && 
				pAndroidUserItem->GetAndroidParameter()==&pAndroidParameter->AndroidParameter)
			{
				//变量定义
				...
				{
					if(bLogoutSuccessed==false)
					{
						bLogoutSuccessed=true;
		...
		else
		{
			if(pAndroidParameter->bIsValided==false)
			{
				...
				if(wParameterIndex==GetAndroidParameterIndex(pAndroidParameter))
				{
					...
					CTraceService::TraceStringEx(TraceLevel_Info, TEXT("本批次机器人全部退出,该批次将被删除, 批次ID=%d"), pAndroidParameter->AndroidParameter.dwBatchID);
				}
			}
			else
			{
				...
				CTraceService::TraceStringEx(TraceLevel_Info, TEXT("本批次机器人全部退出, 批次ID=%d"), pAndroidParameter->AndroidParameter.dwBatchID);
			}
		}
	...
	if(dwTimerID==IDI_ANDROID_PULSE)		
	{
		...
		while (Position!=NULL)
		{
			...
			{
				CTraceService::TraceStringEx(TraceLevel_Exception, _T("IDI_ANDROID_USER_PULSE(UserID=%u,dwTimerID=%u)"), pAndroidUserItem->GetUserID(), dwTimerID);
				ASSERT(FALSE);
...
bool CAndroidUserManager::SendDataToServer(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	{
		CAndroidUserItem *pAndroidItem=GetAndroidUserItem(LOWORD(dwAndroidID));
		DWORD dwUserID=0;
		bool bOffline=false;
		if (pAndroidItem!=NULL && pAndroidItem->m_pIServerUserItem!=NULL)
		{
			dwUserID=pAndroidItem->GetUserID();
			CTraceService::TraceStringEx(TraceLevel_Exception, _T("CAndroidUserManager::SendDataToServer(UserID=%u(%s),MainCmdID=%u,SubCmdID=%u)")\
				,dwUserID,pAndroidItem->m_pIServerUserItem->GetNickName(),wMainCmdID,wSubCmdID);

		}
		else
		{
			CTraceService::TraceStringEx(TraceLevel_Exception, _T("CAndroidUserManager::SendDataToServer(UserID=0,MainCmdID=%u,SubCmdID=%u)"), wMainCmdID, wSubCmdID);
		}
