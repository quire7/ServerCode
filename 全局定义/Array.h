...
//数组模板类
template <class TYPE, class ARG_TYPE=const TYPE &> class CYSArray
{
	//变量定义
protected:
	TYPE *							m_pData;							//数组指针
...
public:
	...
	//拷贝数组
	VOID Copy(const CYSArray & Src);
	//追加数组
	INT_PTR Append(const CYSArray & Src);
	//获取元素
...
public:
	...
	//插入数据
	VOID InsertAt(INT_PTR nIndex, const CYSArray & Src);
	//插入数据
...
//////////////////////////////////////////////////////////////////////////////////
// CYSArray<TYPE, ARG_TYPE> 内联函数

//是否空组
template<class TYPE, class ARG_TYPE> 
AFX_INLINE bool CYSArray<TYPE, ARG_TYPE>::IsEmpty() const
{
...
//获取数目
template<class TYPE, class ARG_TYPE>
AFX_INLINE INT_PTR CYSArray<TYPE, ARG_TYPE>::GetCount() const
{
...
//增加元素
template<class TYPE, class ARG_TYPE>
AFX_INLINE INT_PTR CYSArray<TYPE,ARG_TYPE>::Add(ARG_TYPE newElement)
{
...
//操作重载
template<class TYPE, class ARG_TYPE>
AFX_INLINE TYPE & CYSArray<TYPE, ARG_TYPE>::operator[](INT_PTR nIndex)
{ 
...
//操作重载
template<class TYPE, class ARG_TYPE>
AFX_INLINE const TYPE & CYSArray<TYPE, ARG_TYPE>::operator[](INT_PTR nIndex) const
{ 
	return GetAt(nIndex);
}

//////////////////////////////////////////////////////////////////////////////////
// CYSArray<TYPE, ARG_TYPE> 外联函数

//构造函数
template<class TYPE, class ARG_TYPE> 
CYSArray<TYPE, ARG_TYPE>::CYSArray()
{
	m_pData=NULL;
	m_nMaxCount=0;
	m_nGrowCount=0;
...
//获取缓冲
template<class TYPE, class ARG_TYPE> 
TYPE * CYSArray<TYPE,ARG_TYPE>::GetData()
{
	return m_pData;
}
...
//获取缓冲
template<class TYPE, class ARG_TYPE> 
const TYPE * CYSArray<TYPE,ARG_TYPE>::GetData() const
{
	return m_pData;
}
...
//拷贝数组
template<class TYPE, class ARG_TYPE> 
VOID CYSArray<TYPE,ARG_TYPE>::Copy(const CYSArray & Src)
{
	//效验参数
	ASSERT(this!=&Src);
	if (this==&Src) return;
...
//追加数组
template<class TYPE, class ARG_TYPE> 
INT_PTR CYSArray<TYPE,ARG_TYPE>::Append(const CYSArray & Src)
{
	//效验参数
	ASSERT(this!=&Src);
...
//获取元素
template<class TYPE, class ARG_TYPE> 
TYPE & CYSArray<TYPE,ARG_TYPE>::GetAt(INT_PTR nIndex)
{
...
//获取元素
template<class TYPE, class ARG_TYPE> 
const TYPE & CYSArray<TYPE,ARG_TYPE>::GetAt(INT_PTR nIndex) const
{
	ASSERT((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)||(nIndex>=m_nElementCount)) AfxThrowInvalidArgException();
...
//获取元素
template<class TYPE, class ARG_TYPE> 
TYPE & CYSArray<TYPE,ARG_TYPE>::ElementAt(INT_PTR nIndex)
{
	ASSERT((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)&&(nIndex>=m_nElementCount)) AfxThrowInvalidArgException();
...
//获取元素
template<class TYPE, class ARG_TYPE> 
const TYPE & CYSArray<TYPE,ARG_TYPE>::ElementAt(INT_PTR nIndex) const
{
	ASSERT((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)&&(nIndex>=m_nElementCount)) AfxThrowInvalidArgException();
...
//设置大小
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::SetSize(INT_PTR nNewSize)
{
	//效验参数
	ASSERT(nNewSize>=0);
...
//设置元素
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::SetAt(INT_PTR nIndex, ARG_TYPE newElement)
{
	ASSERT((nIndex>=0)&&(nIndex<m_nElementCount));
...
//设置元素
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::SetAtGrow(INT_PTR nIndex, ARG_TYPE newElement)
{
	//效验参数
	ASSERT(nIndex>=0);
...
//插入数据
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, const CYSArray & Src)
{
	//效验参数
...
//插入数据
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, ARG_TYPE newElement, INT_PTR nCount)
{
	//效验参数
	ASSERT(nIndex>=0);
	ASSERT(nCount>0);
...
//删除数据
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::RemoveAt(INT_PTR nIndex, INT_PTR nCount)
{
	//效验参数
	ASSERT(nIndex>=0);
	ASSERT(nCount>=0);
...
//删除元素
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::RemoveAll()
{
	if (m_nElementCount>0)
	{
		for (INT_PTR i=0;i<m_nElementCount;i++) (m_pData+i)->~TYPE();
...
//释放内存
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::FreeMemory()
{
...
//申请内存
template<class TYPE, class ARG_TYPE>
VOID CYSArray<TYPE,ARG_TYPE>::AllocMemory(INT_PTR nNewCount)
{
	//效验参数
	ASSERT(nNewCount>=0);
