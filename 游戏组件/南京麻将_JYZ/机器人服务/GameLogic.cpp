...
DWORD CGameLogic::CalculateFanShu( DWORD dwHuPaiKind )
{
	...
	{
		//dwHuPaiFanShu += 60;
		dwHuPaiFanShu += 50;	// 20131012 客户确认
	}
	//对对胡
	...
	{
		//dwHuPaiFanShu += 320;
		dwHuPaiFanShu += 380;	// 20131012 客户确认海底捞无花 -- 次将直接结束
	}
	//地胡
...
{	
	...
	BYTE cbQiDuiType = IsQiDui(cbCardIndexTemp, WeaveItem, cbWeaveCount);
	...
	if (cbQiDuiType > 0)
	{
		...
		//case 3: dwHuPaiKind |= HU_HHSQIDUI;	 break;
		//case 4: dwHuPaiKind |= HU_SQIDUIJNH; break;
		}
		//混一色
...
//快照分析
DWORD CGameLogic::AnalyseKuaiZhao( const BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbItemCount, DWORD dwPreHuPaiKind, tagHuPaiResult &HuPaiResult )
{
	ZeroMemory(&HuPaiResult, sizeof(HuPaiResult));

	DWORD dwHuPaiKind = dwPreHuPaiKind;
	if (cbItemCount == 4)
	{
		//检测碰牌数量
		BYTE cbPengPaiCount = 0;
		for (BYTE i=0; i<cbItemCount; i++)
		{
			if ((WeaveItem[i].dwWeaveKind&WIK_PENG) != 0)
			{
				cbPengPaiCount++;
			}
		}
		...
		if (cbPengPaiCount == 4)
		{
			BYTE cbSameProviderCount = 1;
			for (BYTE i=1; i<cbItemCount; i++)
			{
				if (WeaveItem[i].wProvideUser == WeaveItem[0].wProvideUser)
				{
					cbSameProviderCount++;	// 碰了同一玩家次数
				}
				else
				{
					break;
			...
			//形成快照
			if (cbSameProviderCount >= 3)
			{
				dwHuPaiKind |= HU_DUIDUIHU;
				//dwHuPaiKind |= HU_KUAIZHAO;
				if (cbSameProviderCount == 4)
				{
					dwHuPaiKind |= HU_QQDUDIAO;
				}
				if (IsHunYiSe(cbCardIndex, WeaveItem, cbItemCount))
				{
					dwHuPaiKind |= HU_HUNYISE;
				}
...
//七对
BYTE CGameLogic::IsQiDui( const BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount )
{
