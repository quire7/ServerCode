...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	if(dwShootCount==2)
	{
		...
		if(m_GameLogic.IsIntValue(bCardData,bCount))
		{
	        ...
	        if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_OX_ENABLE,1,0);
		}
	}
	else if(dwShootCount==3)
	{
		...
		if(m_GameLogic.IsIntValue(bCardData,bCount))
		{
			bHaveOx = true;
			CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
	        if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_OX_ENABLE,1,1);
		}
	}
	...
	{
		CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
      	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_OX_ENABLE,0,0);
	}
