...
{
	LONGLONG lGold=0L;
	for (LONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*LONGLONG(pow(10L,i));
	return lGold;
}
...
void CGoldControl::DrawGoldCell(CDC * pDC, int nXPos, int nYPox, LONGLONG lGold)
{
	...
	{
		TCHAR szBuffer[12];
		_sntprintf(szBuffer,sizeof(szBuffer),SCORE_STRING,lGold);
...
void CGoldControl::OnPaint()
{
	...
	//提示信息
	TCHAR szBuffer[50];
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("最多：")SCORE_STRING,m_lMaxGold);
	BackFaceDC.SetTextColor(RGB(200,200,200));
	BackFaceDC.SelectObject(m_pSkinResourceManager->GetDefaultFont());
