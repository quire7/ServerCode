	...
	case IDI_CHECK_BANKER:		//检查上庄
		{
			...
			if (m_wCurrentBanker == INVALID_CHAIR||m_nListUserCount<m_nMinRobotListCount)
			{
				//空庄
				m_nWaitBanker++;
...
VOID CAndroidUserItemSink::ReadConfigInformation(TCHAR szFileName[], TCHAR szRoomName[], bool bReadFresh)
{
	...
	//列表人数
	m_nMinRobotListCount = GetPrivateProfileInt(szRoomName, TEXT("RobotListMinCount"), 3, szConfigFileName);
