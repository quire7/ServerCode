...
INT CGameFrameWindow::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (m_pIGameFrameView!=NULL) 
	{
		...
		////创建控件 have no interface
		//tagControlMargin ControlMargin={106, 126, 8, 20};		
		//m_pIGameFrameView->CreateBroadcastControl(ControlMargin, m_pIGameFrameView->GetGameViewHwnd());
	}

	//调整控件
	RectifyControl(rcNormalSize.Width(),rcNormalSize.Height());

	return 0L;
}

//位置变化
VOID CGameFrameWindow::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
	__super::OnWindowPosChanged(lpwndpos);

	if(lpwndpos->cx!=0 && lpwndpos->cy!=0)
	{
		//m_pIGameFrameView->RectifyBroadcastControl();
	}
}
