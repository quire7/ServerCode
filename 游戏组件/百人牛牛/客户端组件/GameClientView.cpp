...
//位置信息
	...
	DeferWindowPos(hDwp,m_btScoreMoveL,NULL,LifeWidth+58+23+140,TopHeight+594+9+39,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btScoreMoveR,NULL,LifeWidth+58+31+405+288,TopHeight+594+9+39,0,0,uFlags|SWP_NOSIZE);
	...
	//框架按钮
	DeferWindowPos(hDwp,m_btMin,NULL, nWidth - 10-25*3-5*2,2,0,0,uFlags|SWP_NOSIZE);
	...
	DeferWindowPos(hDwp,m_btOption,NULL, nWidth - 10-25*4-5*3,2,0,0,uFlags|SWP_NOSIZE);
	//银行按钮
#ifdef __BANKER___
	m_btBankerDraw.GetWindowRect(&rcJetton);
	DeferWindowPos(hDwp,m_btBankerStorage,NULL,nWidth/2,nHeight/2+228,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankerDraw,NULL,nWidth/2,nHeight/2+228+53,0,0,uFlags|SWP_NOSIZE);
#endif
...
{
	m_ImageUserBack.DrawImage(pDC,2,nHeight-m_ImageUserBack.GetHeight()-3);
	m_ImageChatBack.DrawImage(pDC,nWidth-m_ImageChatBack.GetWidth()-3,nHeight-m_ImageChatBack.GetHeight()-2);
	//上部分
	...
	DrawTextString(pDC,m_strRoomName,RGB(255,255,255),RGB(0,0,0),rectInfo, DT_WORDBREAK|DT_EDITCONTROL|DT_END_ELLIPSIS|DT_SINGLELINE|DT_LEFT);
}
...
void CGameClientView::DrawWinFlags(CDC * pDC)
{
	...
	while ( nIndex != m_nRecordLast && ( m_nRecordLast!=m_nRecordFirst ) && nDrawCount < MAX_FALG_COUNT )
	{
		...
		for (int i=0; i<4; ++i)
		{
			...
			//绘画标识
			m_ImageWinFlags.DrawImage( pDC, nXPos, nYPos, m_ImageWinFlags.GetWidth()/4 , 
...
void CGameClientView::ControlBetDetection( BYTE cbViewIndex, LONGLONG lScoreCount, WORD wChair )
{
	...
	int nListIndex = 0;
	if ( cbViewIndex == ID_TIAN_MEN )
		nListIndex = 0;
	else if ( cbViewIndex == ID_DI_MEN )
		nListIndex = 1;
	else if ( cbViewIndex == ID_XUAN_MEN )
		nListIndex = 2;
	else if ( cbViewIndex == ID_HUANG_MEN )
		nListIndex = 3;
	else
		ASSERT(FALSE);
